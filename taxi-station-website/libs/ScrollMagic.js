...
(function (root, factory) {
  ...
  ScrollMagic.Controller = function (options) {
    ...
    var construct = function () {
      ...
      _options.refreshInterval =
        parseInt(_options.refreshInterval) || DEFAULT_OPTIONS.refreshInterval;
      scheduleRefresh();
    ...
    var updateScenes = function () {
      if (_enabled && _updateScenesOnNextCycle) {
        ...
        if (deltaScroll !== 0) {
          ...
          _scrollDirection =
            deltaScroll > 0 ? SCROLL_DIRECTIONS.f : SCROLL_DIRECTIONS.r;
        }
  ...
  ScrollMagic.Scene = function (options) {
    ...
    Scene.on("shift.internal", function (e) {
      ...
      if (
        (_state === "AFTER" && durationChanged) ||
        (_state === "DURING" && _options.duration === 0)
      ) {
    ...
    var updateRelativePinSpacer = function () {
      if (
        _controller &&
        _pin && // well, duh
        _state === "DURING" && // element in pinned state? // is width or height relatively sized, but not in relation to body? then we need to recalc.
    ...
    this.removePin = function (reset) {
      if (_pin) {
        ...
        if (reset || !_controller) {
          ...
          if (spacerChild.hasAttribute(PIN_SPACER_ATTRIBUTE)) {
